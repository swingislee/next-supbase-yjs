...
export default class SupabaseProvider extends EventEmitter {
  ...
  private subscriptionStatus: string = 'UNSUBSCRIBED';  // Add this line
  ...
  onAwarenessUpdate({ added, updated, removed }: any, origin: any) {
    this.logger("receive a awareness update",added,updated,removed,this.awareness)
  ...
  private async onConnect() {
    ...
    if (data && data[this.config.columnName]) {
      ...
      let updateData = data[this.config.columnName];
      if (!Array.isArray(updateData)) {
          updateData = JSON.parse(updateData);  // Attempt to parse if not already an array
      }
      
      // Convert to Uint8Array
      try {
        const typedArray = Uint8Array.from(updateData.map(Number));  // Ensure 
        this.applyUpdate(typedArray);
        this.logger('Applied update to Yjs');
      } catch (error) {
          this.logger('Error applying update:', error);
      }
    } else {
        this.logger('No data found for column:', this.config.columnName);
    }
    ...
    if (this.awareness.getLocalState() !== null) {
      this.logger('getLocalState',this.awareness.getLocalState());
  ...
  private connect() {
    ...
    if (this.channel) {
      this.channel
        ...
        // .on(REALTIME_LISTEN_TYPES.BROADCAST, { event: 'request_awareness' }, ({ payload }) => {
  ...
  constructor(private doc: Y.Doc, private supabase: SupabaseClient, private config: SupabaseProviderConfig) {
    ...
    if (this.config.resyncInterval || typeof this.config.resyncInterval === 'undefined') {
      ...
      this.resyncInterval = setInterval(() => {
        ...
        if (this.channel && this.subscriptionStatus === 'SUBSCRIBED'){
    ...
    this.on('awareness', (update) => {      
      if (this.channel && this.subscriptionStatus === 'SUBSCRIBED'){
        this.logger("received&send the awareness update")
        this.channel.send({
          type: 'broadcast',
          event: 'awareness',
  ...
  public destroy() {
    ...
    if (this.resyncInterval) {
      clearInterval(this.resyncInterval as unknown as number);
    }
